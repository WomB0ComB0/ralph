...
compute_project_hash() {
    ...
    if is_git_repo; then
        ...
        local _ex_names=() _prune=() _n _find_prune=()
        mapfile -t _ex_names < <(hash_exclude_names)
        for _n in "${_ex_names[@]+"${_ex_names[@]}"}"; do
            [[ ${#_prune[@]} -gt 0 ]] && _prune+=(-o)
            _prune+=(-name "$_n")
        done
        # Only add the prune group when non-empty; `find ... \( \)` is a syntax error.
        if [[ ${#_prune[@]} -gt 0 ]]; then
            _find_prune=( -type d \( "${_prune[@]}" \) -prune -o )
        fi

        hash=$(
            find "$project_dir" "${_find_prune[@]+"${_find_prune[@]}"}" -type f -print0 2>/dev/null | \
            sort -z | \
            xargs -0 "$MD5_COMMAND" 2>/dev/null | \
            "$MD5_COMMAND" 2>/dev/null | \
            awk '{print $1}'
...
run_in_sandbox() {
    ...
    if [[ -n "${RALPH_SANDBOX_ALLOW_ENV:-}" ]]; then
        local _extra_env=() _ev
        IFS=$' \t\n,' read -r -a _extra_env <<< "$RALPH_SANDBOX_ALLOW_ENV" || true
        for _ev in "${_extra_env[@]+"${_extra_env[@]}"}"; do
            # Valid identifier name only, and never loader/path-hijack vars, so an
            # extended allowlist can't reintroduce LD_PRELOAD/PATH-style names.
            if [[ "$_ev" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] \
               && ! [[ "$_ev" =~ ^(PATH|IFS|ENV|BASH_ENV|SHELLOPTS|LD_.*|DYLD_.*)$ ]]; then
                safe_env_vars+=("$_ev")
            else
                log_warning "Ignoring unsafe sandbox env var name: $_ev"
            fi
        done
    fi
